...
#define FILESYSTEM_CREATE_TEST_FILES 0
...
int storage_init(void)
{
    ...
    ret = create_dir_if_not_exists(PATH_CAT("root"));
    if (ret != 0) {
        app_log_error("Failed to create root directory: %d\r\n", ret);
        return ret;
    }
...
{
    int ret;
    // uint8_t ssid_buf[21] = {0};
    // uint8_t password_buf[21] = {0};

    uint8_t ssid_len = strlen(ssid);
    uint8_t pwd_len = strlen(password);
    app_log_info("Saving WiFi: SSID='%s' (%d), PWD='%s' (%d)\r\n", 
                ssid, ssid_len, password, pwd_len);

    if (ssid_len > 20) {
        app_log_error("SSID too long: %d characters (max 20)\r\n", ssid_len);
        return -1;
    }
    
    if (pwd_len > 20) {
        app_log_error("Password too long: %d characters (max 20)\r\n", pwd_len);
        return -1;
    }

    // 写入SSID
    // ssid_buf[0] = ssid_len;
    // memcpy(&ssid_buf[1], ssid, ssid_len);
    ret = syscfg_write(VM_WIFI_SSID_ADDR, (void *)ssid, strlen(ssid) + 1);
    if (ret < 0) {
        app_log_error("Failed to write WiFi SSID: %d\r\n", ret);
        return ret;
    }
    
    // 写入密码
    // password_buf[0] = pwd_len;
    // memcpy(&password_buf[1], password, pwd_len);
    ret = syscfg_write(VM_WIFI_PWD_ADDR, (void *)password, strlen(password) + 1);
    if (ret < 0) {
        app_log_error("Failed to write WiFi password: %d\r\n", ret);
        return ret;
    }

    // app_log_info("WiFi credentials saved successfully\r\n");
	return 0;
}
...
{
    int ret;
    char ssid_data[21] = {0};      // 长度 + 20字符
    char pwd_data[21] = {0};       // 长度 + 20字符
    
    // 读取SSID数据（包含长度）
    ret = syscfg_read(VM_WIFI_SSID_ADDR, ssid_data, sizeof(ssid_data));
    if (ret < 0) {
        app_log_error("Failed to read WiFi SSID: %d\r\n", ret);
        ssid_data[0] = 0;
    }
    
    // 读取密码数据（包含长度）
    ret = syscfg_read(VM_WIFI_PWD_ADDR, pwd_data, sizeof(pwd_data));
    if (ret < 0) {
        app_log_error("Failed to read WiFi password: %d\r\n", ret);
        pwd_data[0] = 0;
    }
    
    strncpy(ssid_buf, ssid_data, 20);
    ssid_buf[20] = '\0';
    
    strncpy(password_buf, pwd_data, 20);
    password_buf[20] = '\0';
    
    uint8_t ssid_len = strlen(ssid_buf);
    uint8_t pwd_len = strlen(password_buf);
