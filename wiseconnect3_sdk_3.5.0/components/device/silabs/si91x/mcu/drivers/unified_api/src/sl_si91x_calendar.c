...
sl_status_t sl_si91x_calendar_convert_unix_time_to_calendar_datetime(uint64_t unix_time_ms,
                                                                     sl_calendar_datetime_config_t *cal_date_time)
{
  sl_status_t status        = SL_STATUS_OK;
  uint16_t full_year        = 0;
  ...
  if ( cal_date_time == NULL ) {
    return SL_STATUS_NULL_POINTER;
  }
  

  do {
    // if (!is_valid_time(unix_time_ms, TIME_FORMAT_UNIX, TIME_ZONE_DEFAULT)) {
    //   status = SL_STATUS_INVALID_PARAMETER;
    //   break;
    // }

    cal_date_time->MilliSeconds = unix_time_ms % (MAXIMUM_MILLISECONDS + 1);
    total_seconds               = unix_time_ms / (MAXIMUM_MILLISECONDS + 1);

    // Calculate time components (seconds to hours)
    cal_date_time->Second       = total_seconds % (MAXIMUM_SECOND + 1);
    total_seconds /= (MAXIMUM_SECOND + 1);
    cal_date_time->Minute = total_seconds % (MAXIMUM_MINUTE + 1);
    total_seconds /= (MAXIMUM_MINUTE + 1);
    cal_date_time->Hour = total_seconds % (MAXIMUM_HOUR + 1);
    days_since_epoch = total_seconds / (MAXIMUM_HOUR + 1); // time is now the number of days since TIME_UNIX_EPOCH

    // Calculate day of week (1970-01-01 was Thursday)
    cal_date_time->DayOfWeek = (days_since_epoch + Thursday) % MAXIMUM_DAYS_IN_A_WEEK;

    // Year approximation
    full_year = days_since_epoch / TIME_DAY_PER_YEAR;
    current_year = full_year + base_year;

    // Validate year range (1970 - 25599)
    if (current_year < TIME_UNIX_EPOCH || current_year > 25599) {
      status = SL_STATUS_INVALID_PARAMETER;
      break;
    }
    ...
    if (full_year > (TIME_FIRST_LEAP_YEAR_FROM_UNIX_EPOCH - TIME_UNIX_EPOCH)) {
      leap_day = number_of_leap_days((uint32_t)base_year, (uint32_t)current_year);
      full_year = (days_since_epoch - leap_day) / TIME_DAY_PER_YEAR;
      current_year = full_year + base_year;
      leap_day = number_of_leap_days((uint32_t)base_year, (uint32_t)current_year);
    }

    // Century and year calculation
    years_since_1900 = (uint32_t)(current_year - TIME_NTP_EPOCH); // 1900
    cal_date_time->Century = (years_since_1900 / (MAXIMUM_YEAR + 1)) % (MAXIMUM_CENTURY + 1) + 1;
    cal_date_time->Year = current_year % (MAXIMUM_YEAR + 1);

    // Leap year check
    leap_year_flag = is_leap_year(cal_date_time->Year, cal_date_time->Century) ? 1 : 0;

    // Calculate month and day
    uint64_t day_of_year = days_since_epoch - leap_day - (TIME_DAY_PER_YEAR * full_year);
    while (day_of_year >= days_in_month[leap_year_flag][current_month]) {
      day_of_year -= days_in_month[leap_year_flag][current_month];
      current_month++;
    }
    cal_date_time->Month = (RTC_MONTH_T)(current_month + 1);
    cal_date_time->Day = day_of_year + 1;
  } while (false);
                                                                     ...
                                                                     uint64_t *unix_time_ms)
{
  ...
  do {
    ...
    if (cal_date_time->MilliSeconds > MAXIMUM_MILLISECONDS) { // 确保 MAXIMUM_MILLISECONDS=999
      status = SL_STATUS_INVALID_PARAMETER;
      break;
    }
    ...
    // 添加毫秒计算
    total_milliseconds = (full_year * (TIME_SEC_PER_YEAR * 1000ULL)); //60*60*24*365=31536000ULL

    total_milliseconds += month_days *(86400u * 1000ULL); // TIME_MSEC_PER_DAY=86400000ULL

    total_milliseconds += (cal_date_time->Hour * 3600u * 1000ULL) +
                          (cal_date_time->Minute * 60u * 1000ULL) +
                          (cal_date_time->Second * 1000ULL) +
                           cal_date_time->MilliSeconds;  // 直接添加毫秒值
    *unix_time_ms = total_milliseconds;
  } while (false);
