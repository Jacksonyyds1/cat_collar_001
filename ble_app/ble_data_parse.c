...
static int store_wifi_credentials(char *data, int len) __attribute__((unused));
static int get_wifi_credentials(char *data, int len) __attribute__((unused));
static int get_wifi_status(char *data, int len) __attribute__((unused));
...
static cmd_jump_entry_t command_jump_table[] = {
	...
	{STORE_WIFI_CREDENTIALS, store_wifi_credentials, STORE_WIFI_CREDENTIALS_FRAME_LEN},
	{GET_WIFI_CREDENTIALS, get_wifi_credentials, GET_WIFI_CREDENTIALS_FRAME_LEN},
	...
	{GET_WIFI_STATUS, get_wifi_status, GET_WIFI_STATUS_FRAME_LEN},
	// // CHEKR_CONT_REC
};
...
static int get_epoch_rtc(char *data, int len)
{
	...
	uint64_t currentmillis = sys_cpu_to_be64(get_unix_timestamp());
...
static int set_epoch_rtc(char *data, int len)
{
	...
	// 4. 设置系统时间
	if (set_unix_timestamp(currentmillis) != 0){
		epoch_sync_status = 2;
	}
...
// 0x1c
static int store_wifi_credentials(char *data, int len)
{
    (void)len;
	typedef struct __attribute__((__packed__)) {
		frame_format_header_t header;
		...
		uint16_t crc;
	} store_wifi_cred_size_t;

	store_wifi_cred_size_t *p = (store_wifi_cred_size_t *)data;
	char ssid[21] = {0};
    char password[21] = {0};

	int ssid_start_index = 20 - p->wifi_ssid_length;
    memcpy(ssid, &p->wifi_ssid[ssid_start_index], p->wifi_ssid_length);
    ssid[p->wifi_ssid_length] = '\0';

	int password_start_index = 20 - p->wifi_password_length;
    memcpy(password, &p->wifi_password[password_start_index], p->wifi_password_length);
    password[p->wifi_password_length] = '\0';

	app_log_debug("Received WiFi credentials - SSID: %s, Password: %s\r\n", ssid, password);

	storage_save_wifi_credentials(ssid, password);
	wifi_app_set_event(WIFI_EVENT_STA_RECONNECT);
	...
	typedef struct __attribute__((__packed__)) {
		...
		uint8_t status;
		uint16_t crc;
	} store_wifi_cred_resp_t;

	store_wifi_cred_resp_t resp = {
		.header.start_byte = SB_DEVICE_TO_MOBILE_APP,
		.header.frame_len = sizeof(store_wifi_cred_resp_t),
		.header.frame_type = FT_REPORT,
		.header.cmd = STORE_WIFI_CREDENTIALS,
		.ack = ERR_NONE,
		.status = SYNC_SUCCESS,
	};
...
static int get_wifi_credentials(char *data, int len)
{
	(void)data;
	(void)len;
	...
	storage_get_wifi_credentials(ssid, password);
	uint8_t wifi_status = wifi_app_get_connection_state();

	app_log_debug("WiFi credentials - SSID: %s(%d), Password: %s(%d), connect status: %s\r\n",
		ssid, (int)strlen(ssid), password, (int)strlen(password),
	...
	typedef struct __attribute__((__packed__)) {
		...
		uint16_t crc;
	} get_wifi_cred_resp_t;

	get_wifi_cred_resp_t resp = {
		.header.start_byte = SB_DEVICE_TO_MOBILE_APP,
		.header.frame_len = sizeof(get_wifi_cred_resp_t),
		.header.frame_type = FT_REPORT,
		.header.cmd = GET_WIFI_CREDENTIALS,
		.ack = ERR_NONE,
		.wifi_ssid_length = strlen(ssid),
		.wifi_password_length = strlen(password),
		.wifi_connect_status = wifi_status,
	};

	int ssid_padding = 20 - resp.wifi_ssid_length;
    memset(resp.wifi_ssid, 0, 20);
    memcpy(&resp.wifi_ssid[ssid_padding], ssid, resp.wifi_ssid_length);

    int pwd_padding = 20 - resp.wifi_password_length;
    memset(resp.wifi_password, 0, 20);
    memcpy(&resp.wifi_password[pwd_padding], password, resp.wifi_password_length);
// app_log_debug("send=====>\r\n");
// 	app_log_hexdump_info(&resp, sizeof(resp));

	resp.crc = utils_crc16_modbus((const uint8_t *)&resp, sizeof(resp) - sizeof(uint16_t));
	write_to_central((uint8_t *)&resp, sizeof(resp));
	return 0;
}
static int get_wifi_status(char *data, int len)
{
	(void)data;
	(void)len;
	...
	storage_get_wifi_credentials(ssid, password);
	uint8_t wifi_status = wifi_app_get_connection_state();
	int32_t wifi_rssi_value = wifi_app_get_rssi();
	// int32_t wifi_rssi_value = 100;
	
	app_log_debug("WiFi credentials - SSID: %s(%d), connect status: %s, RSSI: %ld\r\n",
	...
	typedef struct __attribute__((__packed__)) {
		...
		uint8_t wifi_ssid[20];
		uint8_t wifi_connect_status;
		uint8_t intenet_connect_status;
		int32_t wifi_rssi;
		uint8_t reserved_1;
		uint8_t reserved_2;
		uint16_t crc;
	} get_wifi_status_resp_t;

	get_wifi_status_resp_t resp = {
		.header.start_byte = SB_DEVICE_TO_MOBILE_APP,
		.header.frame_len = sizeof(get_wifi_status_resp_t),
		.header.frame_type = FT_REPORT,
		.header.cmd = GET_WIFI_STATUS,
		.ack = ERR_NONE,
		.wifi_ssid_length = strlen(ssid),
		.wifi_connect_status = wifi_status,
		.intenet_connect_status = wifi_status,
        ...
        if(xQueueReceive(bluetooth_data_queue, (void *)&command_buffer, 10 / portTICK_RATE_MS) == pdPASS)
		{
            ...
            for (unsigned int i = 0; i < sizeof(command_jump_table) / sizeof(cmd_jump_entry_t); i++) {
                ...
                if (header->cmd == command && func) {
                    ...
                    app_log_debug("calling func for command: 0x%02x\r\n", command);
...
void command_single_test_handler()
{
    // uint8_t cmd_0x10_start_data[] = {
	// 	0x01, 0x0F, 0xF1, //帧头
	// 	0x10, 			  //命令码
	// 	0x01, 			  //开始/停止记录
	// 	0x00, 0x00, 0x01, 0x99, 0x59, 0x79, 0xDA, 0x23	//时间戳uid，作为文件名 
	// 	};

	// start_stop_rec_session((char *)cmd_0x10_start_data, sizeof(cmd_0x10_start_data));

	// uint8_t cmd_0x06_start_data[] = {
	// 	0x01, 0x0E, 0xF1, //帧头
	// 	0x06, 			  //命令码
	// 	0x00, 0x00, 0x01, 0x99, 0x40, 0xD0, 0x3F, 0x5B	//时间戳uid，作为文件名 00 00 01 99 40 D0 3F 5B 70 8B
	// 	};
	// set_epoch_rtc((char *)cmd_0x06_start_data, sizeof(cmd_0x06_start_data));

	// uint8_t cmd_0x1c_start_data[] = {
	// 	0x01, 0x30, 0xF1, //帧头
	// 	0x1C, 			  //命令码
	// 	0x06, 			  //SSID长度
	// 	0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x63, 0x6F, 0x6F, 0x70, 0x65, 0x72,  //ssid
	// 	0x08, 			  //PWD长度
	// 	0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x31, 0x32, 0x33, 0x34, 0x38, 0x38, 0x38, 0x38,	 //pwd
	// 	};
	// store_wifi_credentials((char *)cmd_0x1c_start_data, sizeof(cmd_0x1c_start_data));

	// uint8_t cmd_0x1d_start_data[] = {
	// 	0x01, 0x07, 0xF1, //帧头
	// 	0x1D, 			  //命令码
	// 	0x00 			  //Reserved
	// 	};
	// get_wifi_credentials((char *)cmd_0x1d_start_data, sizeof(cmd_0x1d_start_data));

	uint8_t cmd_0x24_start_data[] = {
		0x01, 0x07, 0xF1, //帧头
		0x24, 			  //命令码
		0x00 			  //Reserved
		};
	get_wifi_status((char *)cmd_0x24_start_data, sizeof(cmd_0x24_start_data));
	...
	// uint8_t cmd_0x10_start_data[] = {
		// 	0x01, 0x0F, 0x10, //帧头
		// 	0x01, 	
		// 	0x00, 0x00, 0x01, 0x99, 0x59, 0x46, 0x74, 0x37
		// };
	// read_rec_session_details_activity((char *)cmd_0x10_start_data, sizeof(cmd_0x10_start_data));
	app_log_debug("command_single_test_handler\r\n");

}
void command_double_test_handler() 
{
	uint8_t cmd_0x1c_start_data[] = {
		0x01, 0x30, 0xF1, //帧头
		0x1C, 			  //命令码
	...
	app_log_debug("command_double_test_handler\r\n");
}
