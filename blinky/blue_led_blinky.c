...
//                                         SL_SLEEPTIMER_NO_HIGH_PRECISION_HF_CLOCKS_REQUIRED_FLAG);
	...
	// 创建定时器属性
  	osTimerAttr_t timer_attr = { .name = "led_toggle_timer" };
	// 创建定时器
    osTimerId_t led_timer_id = osTimerNew(on_timer_callback, osTimerPeriodic, NULL, &timer_attr);

    if (led_timer_id == NULL) {
        // 定时器创建失败
        return;
    }
	// 启动定时器，周期为10个tick (10ms)
    osStatus_t status = osTimerStart(led_timer_id, 10);
	if (status != osOK) {
        app_log_error("Failed to start timer! Error: %d\n", status);
        return;
    }
	// osTimerStart(blue_led_timer, HANDLE_TOGGLE_DELAY_MS);
}
